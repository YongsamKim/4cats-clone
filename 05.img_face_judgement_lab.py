...
def detect_face(model, cascade_filepath, image):
    ...
    # 500 이상 이미지일때 비율 유지하면서 가로를 500으로 축소
...
def detect_who(model, face_image):
    ...
    print(f"예측결과: {result}")
    # 아메숏, 코숏, 러시안블루, 샴일 가능성을 출력
    print(f"아메숏일 가능성:{result[0][0]*100:.3f}%")
    print(f"코숏일 가능성:{result[0][1]*100:.3f}%")
    print(f"러시안블루일 가능성:{result[0][2]*100:.3f}%")
    print(f"샴일 가능성:{result[0][3]*100:.3f}%")
    # 이름 반환
...
def main():
    print("===================================================================")
    print("Keras를 이용한 고양이 얼굴인식")
    print("학습 모델과 지정한 이미지 파일을 기본으로 고양이 구분하기")
    print("===================================================================")
