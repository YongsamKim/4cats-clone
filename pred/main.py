...
def detect(upload_image):
    ...
    image = rotate(Image.open(upload_image))
...
def detect_who(model, face_image):
    ...
    # 결과
    name = ""
    result = f"아메리칸 숏헤어일 가능성:{predicted[0][0]*100:.3f}% / 코리안 숏헤어일 가능성:{predicted[0][1]*100:.3f}% / 러시안 블루일 가능성:{predicted[0][2]*100:.3f}% / 샴 고양이일 가능성:{predicted[0][3]*100:.3f}%"
